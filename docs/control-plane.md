...
O Control Plane existe desde o bootstrap e usa FastAPI. Ele escuta em `127.0.0.1` por padrão e não deve abrir automaticamente para toda a rede. O comando `eitri web` respeita a configuração `control_plane.allow_public_bind` antes de aceitar outro bind.
