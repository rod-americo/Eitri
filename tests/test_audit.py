...
def test_bootstrap_audit_reports_all_requirements_passed() -> None:
    ...
    assert statuses["tyr_applied_migration"] == "passed"
    assert statuses["thor_real_worker"] == "passed"
    assert report.complete is True


def test_audit_cli_reports_summary_and_strict_passes_after_bootstrap() -> None:
    runner = CliRunner()
    ...
    assert "external_pending': 0" in normal.output
